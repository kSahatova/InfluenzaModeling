...
class AgeModel(BRModel):
    def __init__(self, M, pop_size, incidence_type, age_groups, strains):
        ...
        self.history_states = [["Exposure", "No exposure"] for _ in range(len(self.age_groups))]
    ...
    def make_simulation(self):
        ...
        y = np.zeros((age_groups_num, self.N + 1))  # strains x N
        y[:, 0] = self.I0
        x = np.zeros((self.history_states_num, self.N + 1))  # history_states x N
        x[:, 0] = [(exp_fraction * self.rho).item() for exp_fraction in self.exposed_fraction_h]

        population_immunity = np.zeros((age_groups_num, self.N + 1))  # strains x N
